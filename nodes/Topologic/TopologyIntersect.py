...
def processItem(item):
	...
	topologyC = None
	try:
		topologyMerge = topologyA.Merge(topologyB, tranDict)
		topologySymDiff = topologyA.XOR(topologyB, tranDict)
		topologyC = fixTopologyClass(topologyMerge.Difference(topologySymDiff, tranDict))
	except:
		print("ERROR: (Topologic>Topology.Intersect) operation failed.")
		topologyC = None
	return topologyC
...
class SvTopologyIntersect(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Creates a Topology representing the Boolean Intersection of the two input Topologies
	"""
	...
	def process(self):
		...
		print("Intersect Operation consumed "+str(round(end - start,2))+" seconds")
