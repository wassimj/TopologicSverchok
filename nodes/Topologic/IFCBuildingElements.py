  ...
  def process(self):
    ...
    for ifc_file in ifc_files:
      ...
      for ifc_building_element in ifc_file.by_type("IfcBuildingElement"):
        if not any([ ifc_building_element.is_a(ifc_class) for ifc_class in ["IfcWall", "IfcSlab"] ]):
          continue

        building_element_cell = ifc_topologic.getIfcProductCell(ifc_building_element)
        topologic_lib.setDictionary(building_element_cell, "IfcBuildingElement", ifc_building_element.GlobalId)
        top_building_element_cells.append(building_element_cell)
