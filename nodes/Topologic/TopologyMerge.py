...
def processItem(item):
	...
	topologyC = None
	try:
		topologyC = fixTopologyClass(topologyA.Merge(topologyB, tranDict))
	except:
		print("ERROR: (Topologic>Topology.Merge) operation failed.")
		topologyC = None
	return topologyC
...
class SvTopologyMerge(bpy.types.Node, SverchCustomTreeNode):
	...
	def process(self):
		...
		for i in range(len(tranDictList), maxLength):
			tranDictList.append(topologyBList[-1])
		inputs = []
		outputs = []
		...
		print("Merge Operation consumed "+str(round(end - start,2))+" seconds")
