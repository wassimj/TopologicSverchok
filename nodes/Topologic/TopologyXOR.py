...
class SvTopologyXOR(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Creates a Topology representing the Boolean XOR of the two input Topologies
	"""
	bl_idname = 'SvTopologyXOR'
	bl_label = 'Topology.XOR'
	...
	def process(self):
		...
		if (len(topologyAList) == len(topologyBList)):
			...
			for aTopologyPair in topologies:
				try:
					resultList.append(topologic.Topology.XOR(aTopologyPair[0], aTopologyPair[1]))
				except:
...
def register():
    bpy.utils.register_class(SvTopologyXOR)

def unregister():
    bpy.utils.unregister_class(SvTopologyXOR)
