...
def processItem(item):
	...
	_ = topologic.FaceUtility.AdjacentCells(face, parent, cells)
	return list(cells)

replication = [("Trim", "Trim", "", 1),("Iterate", "Iterate", "", 2),("Repeat", "Repeat", "", 3),("Interlace", "Interlace", "", 4)]

class SvFaceAdjacentCells(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Outputs the adjacent Cells of the input Face
	"""
	bl_idname = 'SvFaceAdjacentCells'
	bl_label = 'Face.AdjacentCells'
	...
	def process(self):
		if not any(socket.is_linked for socket in self.outputs):
			return
		vertexList = self.inputs['Face'].sv_get(deepcopy=False)
...
def register():
	bpy.utils.register_class(SvFaceAdjacentCells)

def unregister():
	bpy.utils.unregister_class(SvFaceAdjacentCells)
