...
def processItem(item):
	...
	_ = topologic.EdgeUtility.AdjacentFaces(edge, parent, faces)
	return list(faces)
...
class SvEdgeAdjacentFaces(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Outputs the adjacent Faces of the input Edge
	"""
	...
	def process(self):
		if not any(socket.is_linked for socket in self.outputs):
			return
		vertexList = self.inputs['Edge'].sv_get(deepcopy=False)
