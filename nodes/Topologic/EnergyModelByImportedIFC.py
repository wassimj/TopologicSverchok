...
def createInnerBoundary(ifc_file, ifc_rel_space_boundary, top_opening_element, top_space_boundary, reverse):
      ...
      for idx, face in enumerate(faces):
        ...
        for other_idx, other_face in enumerate(faces[idx+1:]):
          ...
          for top_opening_element in top_opening_elements:
            ifc_inner_boundary = createInnerBoundary(ifc_file, ifc_rel_space_boundary, top_opening_element, top_space_boundary, True)
            other_ifc_inner_boundary = createInnerBoundary(ifc_file, other_ifc_rel_space_boundary, top_opening_element, other_top_space_boundary, False)
...
class SvEnergyModelByImportedIFC(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Creates an Energy Model from the input IFC file   
	"""
