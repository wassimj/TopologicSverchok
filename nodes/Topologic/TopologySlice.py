...
class SvTopologySlice(bpy.types.Node, SverchCustomTreeNode):
	"""
	Triggers: Topologic
	Tooltip: Creates a Topology representing the Boolean Slice of the two input Topologies
	"""
	bl_idname = 'SvTopologySlice'
	bl_label = 'Topology.Slice'
	...
	def process(self):
		...
		if (len(topologyAList) == len(topologyBList)):
			...
			for aTopologyPair in topologies:
				try:
					resultList.append(topologic.Topology.Slice(aTopologyPair[0], aTopologyPair[1]))
				except:
					print("Error: Slice Operation Failed!")
					continue
			self.outputs['Topology'].sv_set([resultList])


def register():
    bpy.utils.register_class(SvTopologySlice)

def unregister():
    bpy.utils.unregister_class(SvTopologySlice)
